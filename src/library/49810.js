...
export class A {
	static InstrumentLink(e, t, n = null) {
		const r = A.GetLinkParam(t, n);
		return A.AddNavParamToURL(e, r);
	}
	static ParseSNR(e) {
		const t = e.split("_") || [];
		return {
			...
			depth: t[5] ? Number(t[5]) : undefined,
		};
	}
	static GetLinkParam(e, t = null) {
		let n = A.ComputeLinkPrefix(e.domain, e.controller, e.method, e.submethod);
		t = t ?? e.depth ?? null;
	...
	static ComputeLinkPrefix(e, t, n, r) {
		let i = "";
		i += A.EncodeEventComponent(e);
		i += "_";
		i += A.EncodeEventComponent(t);
		i += "_";
		i += A.EncodeEventComponent(n);
		i += "_";
		i += A.EncodeEventComponent(r);
		return i;
	}
