...
const IPL_STATUS_SUCCESS = 0;
const IPL_STATUS_FAILURE = 1;
const IPL_STATUS_OUTOFMEMORY = 2;
const IPL_STATUS_INITIALIZATION = 3;
const IPL_LOGLEVEL_INFO = 0;
const IPL_LOGLEVEL_WARNING = 1;
const IPL_LOGLEVEL_ERROR = 2;
const IPL_LOGLEVEL_DEBUG = 3;
const IPL_SIMDLEVEL_SSE2 = 0;
const IPL_SIMDLEVEL_SSE4 = 1;
const IPL_SIMDLEVEL_AVX = 2;
const IPL_SIMDLEVEL_AVX2 = 3;
const IPL_SIMDLEVEL_AVX512 = 4;
const IPL_SIMDLEVEL_NEON = 0;
const SIZEOF_IPLContextSettings = 20;
...
class IPLVector3 {
	...
	static calcRelativeDirection(e, t, i, r, o, s) {
		...
		const a = IPLVector3.dot(s, r);
		const l = -IPLVector3.dot(s, i);
		s.x = n;
		s.y = a;
		s.z = l;
		s.normalize();
...
const SIZEOF_IPLAudioBuffer = 12;
...
class IPLAudioBuffer {
	constructor(e, t, i) {
		...
		if (r == 0) {
			...
			console.error("Unable to allocate audio buffer:", r);
		}
	}
	destroy() {
	...
	copyFrom(e) {
		...
		for (let i = 0; i < t; ++i) {
			this.channels[i].set(e[i]);
		}
	}
	copyTo(e) {
		const t = Math.min(this.numChannels, e.length);
		for (let i = 0; i < t; ++i) {
			e[i].set(this.channels[i]);
		}
	}
	mix(e) {
...
const IPL_HRTFTYPE_SOFA = 1;
const IPL_HRTFNORMTYPE_NONE = 0;
const IPL_HRTFNORMTYPE_RMS = 1;
const SIZEOF_IPLHRTFSettings = 24;
...
const IPL_HRTFINTERPOLATION_BILINEAR = 1;
const SIZEOF_IPLBinauralEffectSettings = 4;
const SIZEOF_IPLBinauralEffectParams = 28;
...
class SteamAudioBinauralProcessor extends AudioWorkletProcessor {
	...
	static staticInit(e) {
		...
		this.audioSettings.samplingRate = 48000;
