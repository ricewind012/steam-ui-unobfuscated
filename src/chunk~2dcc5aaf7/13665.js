...
class A {
	...
	async UploadPendingData() {
		...
		if (this.m_CMInterface) {
			...
			let n = `${xv()}actions/ajaxmarkpartnerevents`;
...
Cg([m.o], A.prototype, "UploadPendingData", null);
export let Nh;
class C {
	clanid = undefined;
	appid = 0;
...
Cg([s.sH], C.prototype, "clanid", undefined);
Cg([s.sH], C.prototype, "appid", undefined);
Cg([s.sH], C.prototype, "can_edit", undefined);
Cg([s.sH], C.prototype, "owns_app", undefined);
Cg([s.sH], C.prototype, "follows_app", undefined);
Cg([s.sH], C.prototype, "support_user", undefined);
Cg([s.sH], C.prototype, "valve_admin", undefined);
Cg([s.sH], C.prototype, "limited_user", undefined);
Cg([s.sH], C.prototype, "event_ignored", undefined);
Cg([s.sH], C.prototype, "event_followed", undefined);
Cg([s.sH], C.prototype, "event_followed_flags", undefined);
((e) => {
...
export class KN {
	...
	async Init(e) {
		...
		const t = Fd("partnereventpermissions", "application_config");
		...
		let r = Fd("uservotes", "application_config");
		if (r) {
			h5(() => {
				r.forEach((e) => {
					let t = !!e.voted_up || !e.voted_down;
	...
	BFollowsEvent(e, t) {
		return this.GetPartnerEventPermissions(e).event_followed.includes(t);
	}
	...
	BIgnoresEvent(e, t) {
		return this.GetPartnerEventPermissions(e).event_ignored.includes(t);
	}
	...
	async InternalLoadSingleAppEventPermissions(e) {
		...
		try {
			if (Boolean(u.iA.logged_in)) {
				...
				}gid/${e.ConvertTo64BitString()}/ajaxgetpartnereventpermissions/`;
				let s = {};
				if (yK() == "partnerweb") {
					i = `${u.TS.PARTNER_BASE_URL}partnerevents/ajaxgetpartnereventpermissions`;
					s = {
				...
				if (t.data.success != 1) {
					console.error(`Partner Events Failed Load:${H(t.data).strErrorMsg}`);
					n = true;
				} else {
			...
			if (
				...
				const t = H(e);
				console.error(
					`InternalLoadSingleAppEventPermissions account: ${r}: ${t.strErrorMsg}`,
					t,
				);
			}
		} finally {
	...
	async SetFollowOrUnfollowEvent(e, t, r, n, i) {
		...
		h5(() => {
			...
			if (m) {
				...
				o = t || e.result.event_followed_flags;
			} else if (t) {
		...
		}
		{
			...
			const i =
				n == "community" || n == "steamtv"
					? `${
							u.TS.COMMUNITY_BASE_URL
						}gid/${e.clanSteamID.ConvertTo64BitString()}/announcements/rate/${
							e.AnnouncementGID
						}`
	...
	async LoadMyVote(e, t) {
		if (e?.AnnouncementGID) {
			...
			if (this.m_cm) {
				...
				if (n.GetEResult() == 1) {
					r = !!n.Body().voted_up() || !n.Body().voted_down();
				}
			} else {
				const n = yK() == "store";
				const i = n
					...
					: `${
							u.TS.COMMUNITY_BASE_URL
						}gid/${e.clanSteamID.ConvertTo64BitString()}/announcements/ajaxgetmyvote/${
							e.AnnouncementGID
						}`;
				const s = {
				...
				r = !!o.data.voted_up || !o.data.voted_down;
			}
...
Cg([s.sH], KN.prototype, "m_mapClanToUserPermissions", undefined);
Cg([s.sH], KN.prototype, "m_mapAnnounceGIDToVote", undefined);
Cg([s.sH], KN.prototype, "m_setReadEventGIDs", undefined);
Cg([s.XI], KN.prototype, "CopyFromResponseToTrack", null);
