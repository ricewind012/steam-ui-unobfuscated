...
function c(e) {
	if (e == O4.LNY2020) {
		return `hue-rotate(${360 + Tg(-30, 10)}deg)`;
	} else {
		return "";
	}
}
	...
	const a = useMemo(() => {
		let n = ((e, t) => {
			...
			for (let e = 0; e < 200; e++) {
				...
				const m = (Math.random() - 0.5) * 4 * Math.PI * 20;
				const u = Math.random() + 0.5;
				const d = [
					Math.random() * height - height - 20,
					height + 20 + Math.random() * height * u,
				];
				const A =
					u * (width <= 1000 ? 1 : width / 1000) * (t == O4.Gold ? 2 : 1);
				const p = (Math.random() - 0.5) * width;
				const g = (Math.random() - 0.5) * width;
				const h = [g, p + g];
				const C = l(t);
				const _ = c(t);
				r.push({
			...
			((e, t, r) => {
				...
				return {
					...
					transform: t.interpolate((e) => {
						const t = e * rotationCoefficient * rotationRatioY;
						const r = e * rotationCoefficient * rotationRatioX;
						const o = e * rotationCoefficient * rotationRatioZ;
						const u = e * (yRange[1] - yRange[0]) + yRange[0];
						return `translate(${
							e * (xRange[1] - xRange[0]) + xRange[0]
						}px, ${u}px) rotateY(${t}rad) rotateX(${r}rad) rotateZ(${o}rad) scale(${scale})`;
					}),
				};
			})(e, anim),
		);
			...
			.map((e) =>
				((e, t, r) => {
					...
					return {
						...
						transform: t.interpolate((e) => {
							const t = ((e * rotationCoefficient) / 4) * rotationRatioZ;
							const r = e * (yRange[1] - yRange[0]) + yRange[0];
							return `translate(${
								e * (xRange[1] - xRange[0]) + xRange[0]
							}px, ${r}px) rotateZ(${t}rad) scale(${scale})`;
						}),
					};
				})(e, anim),
			)
	...
	const [t, r] = (() => {
		...
		const r = useCallback((e) => {
			if (e && typeof e.getBoundingClientRect == "function") {
				setE(e.getBoundingClientRect());
			}
		}, []);
		return [e, r];
	})();
	let i;
	let { rgParticleStyles, rgStreamerStyles } = m(t, eType);
	switch (eType) {
		case O4.Gold: {
			i = rgParticleStyles.map((e, t) => <G key={t} style={e} />);
			break;
		}
		case O4.LNY2020: {
			i = rgParticleStyles.map((e, t) =>
				t % 2 ? <P key={t} style={e} /> : <H key={t} style={e} />,
			);
			break;
		}
		case O4.Default: {
			i = rgParticleStyles.map((e, t) => <P key={t} style={e} />);
		}
	}
	return (
		<div
		...
		>
			{i}
			{rgStreamerStyles.map((e, t) =>
				e.flRandom > 0.5 ? <_ key={t} style={e} /> : <C key={t} style={e} />,
			)}
		</div>
	);
};
...
const G = ({ style }) => {
	const [t] = useState(Math.floor(Math.random() * f.length));
	const FT = f[t];
	return (
		<FT
			style={{
...
const H = ({ style }) => {
	const [t] = useState(Math.floor(Math.random() * b.length));
	const BT = b[t];
	return (
		<BT
			style={{
...
const f = [
	...
	({ style }) => (
		...
		>
			<ellipse cx="24" cy="15" rx="24" ry="15" fill={style.fill} />
		</i.CS.svg>
	),
	({ style }) => (
		<i.CS.svg
	...
	({ style }) => (
		...
		>
			<path d="M48 0H0L24 35L48 0Z" fill={style.fill} />
		</i.CS.svg>
	),
];
const b = [
