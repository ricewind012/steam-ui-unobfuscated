...
var i = require(/*webcrack:missing*/ "./28168.js");
	...
	const a = (0, n.useMemo)(() => {
		let n = (function (e, t) {
			...
			for (let e = 0; e < 200; e++) {
				...
				const d = [Math.random() * i - i - 20, i + 20 + Math.random() * i * u];
				const A = u * (n <= 1000 ? 1 : n / 1000) * (t == O4.Gold ? 2 : 1);
				const p = (Math.random() - 0.5) * n;
				const g = (Math.random() - 0.5) * n;
				const h = [g, p + g];
				const C = l(t);
				const _ = c(t);
				r.push({
			...
			(function (e, t, r) {
				...
				return {
					...
					transform: t.interpolate((e) => {
						...
						const r = e * n * a;
						const o = e * n * s;
						const u = e * (l[1] - l[0]) + l[0];
						return `translate(${e * (c[1] - c[0]) + c[0]}px, ${u}px) rotateY(${t}rad) rotateX(${r}rad) rotateZ(${o}rad) scale(${m})`;
					}),
				};
			})(e, r),
		);
...
export const NW = ({ eType: e }) => {
	...
	let i;
	let { rgParticleStyles: a, rgStreamerStyles: s } = m(t, e);
	switch (e) {
		case O4.Gold:
			i = a.map((e, t) =>
				n.createElement(g, {
...
const g = ({ style: e }) => {
	const [t] = (0, n.useState)(Math.floor(Math.random() * f.length));
	const r = f[t];
...
const h = ({ style: e }) => {
	const [t] = (0, n.useState)(Math.floor(Math.random() * b.length));
	const r = b[t];
	return n.createElement(r, {
		style: {
			...u,
